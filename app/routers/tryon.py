...
async def generate_tryon(
    ...
    # ✅ 生成前检查余额（不扣除）
    if user.credits < 80:
        raise HTTPException(status_code=403, detail="虚拟试穿需要80灵境点，当前余额不足，请充值")
    ...
    # ✅ 生成成功后扣点
    check_and_deduct_credits(user, db, 80, "虚拟试穿")
...
async def generate_tryon_by_url(
    ...
    # ✅ 只有用户登录时才扣点（内部调用免扣点）
    if current_user:
        check_and_deduct_credits(current_user, db, 80, "虚拟试穿")
